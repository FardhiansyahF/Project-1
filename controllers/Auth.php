...
class Auth extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'Login | Wisata Kreatif C59';
            ...
            if ($cek_username > 0) {
                ...
                if (password_verify($input['password'], $password)) {
                    ...
                    if ($user_db['is_active'] != 1) {
                        set_pesan('Akun anda belum aktif. Silahkan hubungi admin!', false);
                        redirect('login');
                    } else {
                        $userdata = [
                    ...
                    set_pesan('Password yang anda masukan salah.', false);
                    redirect('auth');
                }
            } else {
                set_pesan('Username yang anda masukan belum terdaftar.', false);
                redirect('auth');
            }
        }
    }
    ...
    public function register()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($query) {
                ...
                set_pesan('Gagal melakukan registrasi!', false);
                redirect('register');
            }
        }
    }
